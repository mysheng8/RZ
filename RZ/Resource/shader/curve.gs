...
void LineGS(line VS_OUT gIn[2],inout TriangleStream<GS_OUT> triStream)
{
	...
	float3 planeNormal = gIn[1].wpos.xyz - campos;
	//planeNormal.y = 0.0f;
	...
	float3 width1 = sideVec*0.01f; //* strandWidth[0]; 
	float3 width2 = sideVec*0.01f; //* strandWidth[1]; 
	...
	for(int i = 0; i < 4; i++)
	{
		gOut.position = mul(wv[i], worldMatrix);  
		gOut.position = mul(gOut.position, viewMatrix);
